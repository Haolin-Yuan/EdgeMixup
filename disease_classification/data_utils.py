...
from skin_segmentation.data_utils import calc_batch_ITAs, bounded_decision_function
from skin_segmentation.model_utils import predict_mask_generator
import disease_classification.clf_config as clss_config
import skin_segmentation.segmentation_cfg as seg_config
import sys
sys.path.append("../")
...
def clf_im_aug_func(image, mask):
    aug = albumentations.Compose([
        ...
        albumentations.RandomResizedCrop(height=clss_config.IMAGE_SIZE[0], width=clss_config.IMAGE_SIZE[1], scale=(0.75, 1.0), ratio=(0.9, 1.1), p=0.5),
...
class RawDataset:
    ...
    def prune_broken_images(self):
        ...
        for im_pth, lbl in zip(self.im_paths, self.all_labels):
            full_img_pth = str(clss_config.ROOT_CLF_IMG_DIR / im_pth)
    ...
    def __getitem__(self, item_idx):
        img_pth = str(clss_config.ROOT_CLF_IMG_DIR / self.im_paths[item_idx])
...
class RawDataLoader(keras.utils.Sequence):
    ...
    def __getitem__(self, item_idx):
        ...
        for i in range(start, stop):
            ...
            batch_img_paths.append(str(clss_config.ROOT_CLF_IMG_DIR / self.dataset.im_paths[i]))
            batch_labels.append(label)

            if self.preds is not None:
                pred_mask = bounded_decision_function(self.preds[i], threshold=clss_config.BOUNDED_DECISION_THRESHOLD)
                ...
                pred_mask_fname = str(clss_config.CLF_MASK_DIR / self.dataset.dataset_type / base_mask_path)
                if self.save_masks:
...
def load_raw_splits():
    train_df = pd.read_csv(str(clss_config.CLF_RAW_SPLITS_DIR / 'NO_vs_EM_vs_HZ_vs_TC_train.csv'))
    train_df.columns = ['images', 'disease_labels']
    val_df = pd.read_csv(str(clss_config.CLF_RAW_SPLITS_DIR / 'NO_vs_EM_vs_HZ_vs_TC_val.csv'))
    val_df.columns = ['images', 'disease_labels']
    test_df = pd.read_csv(str(clss_config.CLF_RAW_SPLITS_DIR / 'NO_vs_EM_vs_HZ_vs_TC_test.csv'))
    ...
    train_set = RawDataset(
        ...
        image_size=clss_config.IMAGE_SIZE,
...
def gen_preliminary_data():
    ...
    for split in ['train', 'val', 'test']:
        ...
        raw_loader = RawDataLoader(datasets[split], None, batch_size=clss_config.SKIN_SEG_BATCH_SIZE, save_masks=False)
        preds = predict_mask_generator(None, custom_loader=raw_loader, batch_size=clss_config.SKIN_SEG_BATCH_SIZE, make_masks_usable=False)
        (clss_config.CLF_MASK_DIR / split).mkdir(parents=True, exist_ok=True)
        loader = RawDataLoader(datasets[split], preds, batch_size=clss_config.SKIN_SEG_BATCH_SIZE, save_masks=True)
        df = pd.DataFrame(columns=['images', 'pred_mask', 'disease_label', 'pred_ita', 'pred_skintone_category', 'pred_binary_skintone'])  # creates the dataframe
        ...
        csv_name = str(clss_config.CLF_SPLITS_DIR / f"{split}_processed.csv") #str(clss_config.SKIN_TONE_CSV).replace('skin_tones', 'test_skin_tones')
...
class SkinDiseaseDataset(T.utils.data.Dataset):
    def __init__(self, images, labels, pred_mask_paths, pred_binary_skintone, split="all", image_size=clss_config.IMAGE_SIZE,
    ...
    def __getitem__(self, idx):
        img_pth = str(Path(clss_config.ROOT_CLF_IMG_DIR) / self.im_paths[idx])
        img = cv2.cvtColor(cv2.imread(img_pth), cv2.COLOR_BGR2RGB)
        img = cv2.resize(img, (self.im_w, self.im_h))
        ...
        if self.debug:
            ...
            plt.imshow((pred_mask!= clss_config.Label.LESION.value).astype(np.uint8) * 127)
...
class EvalSkinDiseaseDataset():
    def __init__(self, images, labels, pred_mask_paths, pred_binary_skintone, pred_disease, image_size=clss_config.IMAGE_SIZE, mask_images=True, model_type="baseline"):
        self.im_w, self.im_h = image_size
        self.im_paths = images
        self.labels = labels
        self.pred_mask_paths = pred_mask_paths
    ...
    def __getitem__(self, idx):
        img_pth = str(clss_config.ROOT_CLF_IMG_DIR / self.im_paths[idx])
        img = cv2.cvtColor(cv2.imread(img_pth), cv2.COLOR_BGR2RGB)
        img = cv2.resize(img, (self.im_w, self.im_h))
        mask_path = self.pred_mask_paths[idx]# if ".npy" in self.pred_mask_paths[idx] else self.pred_mask_paths[idx] + ".npy"
        pred_mask = np.load(mask_path)
        ...
        if self.mask_images:
            ...
            mask = np.tile(pred_mask, (3, 1, 1)) != clss_config.Label.LESION.value # Repeat the mask across all 3 RGB dimensions
        ...
        pred_disease_class = clss_config.CLF_Label_Translate[top_k_idx.item()]

        error_prediction = self.labels[idx] != top_k_idx.item()

        fig.add_subplot(2, 2, 4)
        plt.imshow(np.transpose(img, (1, 2, 0)).astype(np.uint8))
        #plt.title(f"\nPreprocessed Image. Label: {clss_config.CLF_Label_Translate[self.labels[idx]]} Skintone: {self.pred_binary_skintone[idx]} idx: {idx}\n predicted disease: {pred_disease_class}")
        plt.title(f"\nPreprocessed Image. Label: {clss_config.CLF_Label_Translate[self.labels[idx]]} Skintone: {self.pred_binary_skintone[idx]}\n predicted disease: {pred_disease_class}")
        debug_dir = clss_config.CLF_OUTPUT_DIR / "test_result_images" / self.model_type if not error_prediction else clss_config.CLF_OUTPUT_DIR / "test_result_images" / self.model_type / "error_classifying"
        ...
        result = {
            ...
            "gt": clss_config.CLF_Label_Translate[self.labels[idx]],
...
def load_clean_splits(model_type, edgemixup):
    ...
    train_df = clean_df(pd.read_csv(str(clss_config.CLF_SPLITS_DIR / 'train_processed.csv')))
    val_df = clean_df(pd.read_csv(str(clss_config.CLF_SPLITS_DIR / 'val_processed.csv')))
    test_df = clean_df(pd.read_csv(str(clss_config.CLF_SPLITS_DIR / 'test_processed.csv')))


    if edgemixup:
        train_df = add_segment_boundary(train_df, clss_config.EdgeMixup_class_root, seg_config.MODEL_PATH)
        val_df = add_segment_boundary(val_df, clss_config.EdgeMixup_class_root, seg_config.MODEL_PATH)
        test_df = add_segment_boundary(test_df, clss_config.EdgeMixup_class_root, seg_config.MODEL_PATH)

    mask_images = "mask" in model_type
    datasets = {
        "train": SkinDiseaseDataset(
                    images = train_df['images'].to_list(),
...
def gen_eval_debug_plots(pred_disease, model_type):
    test_df = pd.read_csv(str(clss_config.CLF_SPLITS_DIR / 'test_processed.csv'))
    ...
    for i, result in tqdm(enumerate(debug_plot_dataloader)):
        for idx, correct in enumerate(result['correct']):
            ...
            if num_incorrect_imgs > 16:
                fig.tight_layout()
                debug_dir = clss_config.CLF_OUTPUT_DIR / "test_result_images" / model_type / "error_classifying_montage"
                debug_dir.mkdir(parents=True, exist_ok=True)
    ...
    for i, result in tqdm(enumerate(debug_plot_dataloader)):
        for idx, correct in enumerate(result['correct']):
            ...
            if num_incorrect_imgs > 16:
                fig.tight_layout()
                debug_dir = clss_config.CLF_OUTPUT_DIR / "test_result_images" / model_type / "correct_classifying_montage"
                debug_dir.mkdir(parents=True, exist_ok=True)
                fig.savefig(str(debug_dir / f'test_img_montage_{i}.png'))
                num_incorrect_imgs = 1
                plt.cla()
