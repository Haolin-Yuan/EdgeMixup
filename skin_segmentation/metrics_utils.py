...
from segmentation_models.base.objects import Metric
...
class JaccardScore(Metric):
    ...
    def __call__(self, gt, pr):
        gt_flat = gt.numpy().reshape(-1, len(Label))
        pr_one_hot = tf.one_hot(tf.argmax(pr, axis=-1), depth=len(Label))
        pr_flat = pr_one_hot.numpy().reshape(-1, len(Label))
...
class DICEScore(Metric):
    ...
    def __init__(
        ...
        name = name or 'DICE{}_score'.format(beta)
        super().__init__(name=name)
        self.beta = beta
        self.class_weights = class_weights if class_weights is not None else 1
        self.class_indexes = class_indexes
